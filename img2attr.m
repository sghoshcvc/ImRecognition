function [featSize,numBlockX,numBlockY,fv_Rep] = img2attr( im )
...
PCA = readPCA(fullfile(pathModel,'PCA.bin'));
GMM = readGMM(fullfile(pathModel,'GMM.bin'));
attModels = readMat(fullfile(pathModel,'attModels.bin'));
W = attModels(1:end-1,:);
[Ht,Wd] = size(im);
   ...
   % posAtI = int64(posAtI(1:end-1));
    fileAttributes = fullfile(pathModel,sprintf('fileAttributes.bin'));
    ...
    %fwrite(fid, posAtI, 'int64');
     ...
     [frames, descrs] = vl_phow(im, opts.phowOpts{:}) ;
        ...
        numBlockY = ceil(Ht/BlockSize);
        numBlockX = ceil(Wd/BlockSize);
        Blocks = cell(numBlockY,numBlockX);
        %Blockseach = cell(numBlockX,numBlockY);
       % ImageIntNorm = zeros(numBlockX,numBlockY);
        for j=1:numPoints
            Y = ceil(frames(1,j)/BlockSize);
            X = ceil(frames(2,j)/BlockSize);
            % fprintf('X =%d,Y= %d\n',X,Y);
